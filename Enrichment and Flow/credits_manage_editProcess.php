...
$URL = $session->get('absoluteURL')."/index.php?q=/modules/Enrichment and Flow/credits_manage_edit.php&enfCreditID=$enfCreditID&enfDomainID=$enfDomainID&search=$search";
...
if (isActionAccessible($guid, $connection2, '/modules/Enrichment and Flow/credits_manage_edit.php') == false) {
    ...
    if (!$creditGateway->unique($data, ['name'], $enfCreditID)) {
        $URL .= '&return=error7';
        header("Location: {$URL}");
        exit;
    }
