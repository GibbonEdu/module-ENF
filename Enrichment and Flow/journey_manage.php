...
if (isActionAccessible($guid, $connection2, '/modules/Enrichment and Flow/journey_manage.php') == false || $highestAction == false) {
    // Access denied
        ...
        ->format(function ($category, $actions) {
            if ($category['status'] != 'Current - Pending') {
                ...
                $actions->addAction('accept', __('Accept'))
                    ->setURL('/modules/Enrichment and Flow/journey_manage_commit.php');
            }
            if ($category['status'] != 'Complete - Approved') {
                $actions->addAction('delete', __('Delete'))
                    ->setURL('/modules/Enrichment and Flow/journey_manage_delete.php');
            }
        });
