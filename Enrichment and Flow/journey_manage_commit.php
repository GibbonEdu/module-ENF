...
$highestAction = getHighestGroupedAction($guid, '/modules/Enrichment and Flow/journey_manage_commit.php', $connection2);

if (isActionAccessible($guid, $connection2, '/modules/Enrichment and Flow/journey_manage_commit.php') == false || $highestAction == false) {
    // Access denied
    ...
    echo __m('Please {link1} if you are able to get involved, or, {link2} if you not in a position to help.', array('link1' => "<a class='p-1 border border-solid border-green-500 text-green-500 bg-green-200' href='".$session->get('absoluteURL')."/modules/Enrichment and Flow/journey_manage_commitProcess.php?response=Y&enfJourneyID=$enfJourneyID&statusKey=$statusKey&search=$search&status=$status&gibbonPersonIDStudent=$gibbonPersonIDStudent'>click here</a>", 'link2' => "<a class='p-1 border border-solid border-red-500 text-red-500 bg-red-200' href='".$session->get('absoluteURL')."/modules/Enrichment and Flow/journey_manage_commitProcess.php?response=N&enfJourneyID=$enfJourneyID&statusKey=$statusKey&search=$search&status=$status&gibbonPersonIDStudent=$gibbonPersonIDStudent'>click here</a>"));
    echo '</p>';
}
