...
if (isActionAccessible($guid, $connection2, '/modules/Enrichment and Flow/journey_manage_edit.php') == false || $highestAction == false) {
    ...
    if ($search !='' || $status !='' || $gibbonPersonIDStudent !='') {
        ...
        $page->navigator->addSearchResultsAction(Url::fromModuleRoute('Enrichment and Flow', 'journey_manage.php')->withQueryParams($params));
    }
    ...
    if ($values['status'] != 'Current - Pending') {
        ...
        $form = Form::create('log', $session->get('absoluteURL').'/modules/'.$session->get('module')."/journey_manage_editProcess.php?enfJourneyID=$enfJourneyID&search=$search&status=$status&gibbonPersonIDStudent=$gibbonPersonIDStudent");
