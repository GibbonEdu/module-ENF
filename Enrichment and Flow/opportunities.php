...
if (isActionAccessible($guid, $connection2, '/modules/Enrichment and Flow/opportunities.php') == false) {
    // Access denied
    ...
    ->format(function($values) use ($session, $gibbon, $search, $opportunityCreditGateway) {
        ...
        $return .= "<div title='".str_replace("'", "&#39;", $values['description'])."' class='text-center pb-2'>";
            $return .= "<a class='text-black no-underline' href='".$session->get('absoluteURL')."/index.php?q=/modules/Enrichment and Flow/opportunities_detail.php&enfOpportunityID=".$values['enfOpportunityID']."&search=$search'>";
            ...
            $return .= "<div class='text-xs italic pt-2'>";
                ...
                $return .= "</div>";
                $credits = $opportunityCreditGateway->selectCreditsByOpportunity($values['enfOpportunityID']);
