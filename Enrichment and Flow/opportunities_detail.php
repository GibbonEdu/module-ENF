...
if (isActionAccessible($guid, $connection2, '/modules/Enrichment and Flow/opportunities_detail.php') == false) {
    // Access denied
    ...
    $result = $container->get(OpportunityGateway::class)->selectOpportunityByID($enfOpportunityID);
    ...
    if ($search !='') {
        ...
        $page->navigator->addSearchResultsAction(Url::fromModuleRoute('Enrichment and Flow', 'opportunities.php')->withQueryParams($params));
    }
    ...
    $table->addColumn('mentors', __m('Mentors'))
        ->addClass('col-span-2')
        ->format(function ($values) use ($container, $enfOpportunityID) {
            $return = '';

            $mentors = $container->get(OpportunityMentorGateway::class)->selectMentorsByOpportunity($enfOpportunityID);
