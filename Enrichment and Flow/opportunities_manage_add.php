...
if (isActionAccessible($guid, $connection2, '/modules/Enrichment and Flow/opportunities_manage_add.php') == false) {
    ...
    if (isset($_GET['editID'])) {
        $editLink = $session->get('absoluteURL').'/index.php?q=/modules/Enrichment and Flow/opportunities_manage_edit.php&enfOpportunityID='.$_GET['editID']."&search=$search";
    }
    ...
    $row = $form->addRow();
        ...
        $row->addSelect('enfCreditID')->selectMultiple()->fromQuery($pdo, $sql, array(), 'groupBy');
